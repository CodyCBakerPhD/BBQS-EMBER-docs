...
runs in *this* repository's context — which means `workflow_run` or
`pull_request_target`, both of which can read secrets.

That is the whole reason these two workflows exist, and it is also exactly what
makes them sensitive: they can read a credential with write access to the
previews repository, and they are triggered by events an untrusted party
controls.
...
| `preview-cleanup.yaml` | `pull_request_target` | reads `PREVIEW_DEPLOY_TOKEN` | No — no checkout of pull request code, no build |
...
3. **`check-docs.yaml` and `build-docs.yaml` must stay read-only and
